...
// (mb, ic) * (oc, ic)
void fully_connect_naive_ccc(float* input, float* weight, float* bias,
                             float* output, int MB, int IC, int OC) {
...
// 逐个OC进行求值
void fully_connect_ccc_2(float* input, float* weight, float* bias,
                         float* output, int MB, int IC, int OC) {
  memset(output, 0, OC * sizeof(float));
  memcpy(output, bias, OC * sizeof(float));
  int len   = svcntw();
  int len_2 = 2 * len;
  for (int mb = 0; mb < MB; ++mb) {
...
void fully_connect_ccc(float* input, float* weight, float* bias, float* output,
                       int MB, int IC, int OC) {
  memset(output, 0, OC * sizeof(float));
  memcpy(output, bias, OC * sizeof(float));
  int len   = svcntw();
  int len_2 = 2 * len;
