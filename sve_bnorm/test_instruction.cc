...
void naive(float* src, float* dst, int length) {
  for (size_t i = 0; i < length; i++) { dst[i] = 1 / sqrt(src[i]); }
}
...
int main() {
  ...
  for (size_t i = 0; i < sve_len; i++) { vec[i] = 1.f / float(rand() % 100) + float_const; }
  ...
  for (size_t i = 0; i < sve_len; i++) {
    if (fabs(vec_ans_naive[i] - vec_ans_sve[i]) > 1e5) {
      err++;
      std::cout << vec[i] << " : " << vec_ans_naive[i] << " : " << vec_ans_sve[i] << "\n";
    }
  }
