 ...
 * @FilePath     : /sve/row_major/sve_gemm/main.c
 * @ToDo         :
 */
...
int main(int argc, char **argv) {
  ...
  tic();
  naive_gemm(store_order, Atrans, Btrans, argM, argN, argK, alpha, a, argK, b, argN, beta, c_ref, argN);
  time = toc();
  if (!P) printf("naive : %lf\n", time);

  /* my gemm operation */
  tic();
  my_dgemm(store_order, Atrans, Btrans, argM, argN, argK, alpha, a, argK, b, argN, beta, c, argN);
  time = toc();
  if (!P) printf("my : %lf\n", time);
