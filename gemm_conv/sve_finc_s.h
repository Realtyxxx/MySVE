...
#define LOAD2VEC_CONT_S(vec1, vec2, preg, areg, avec1, avec2) \
  LOAD2VEC_S(vec1, vec2, preg, areg)

#define LOAD2VEC_GENI_S(vec1, vec2, preg, areg, avec1, avec2) \
  " ld1w  " #vec1 ".s, " #preg "/z, [" #areg "," #avec1       \
  ".s, UXTW #2]\n\t"                                          \
  " ld1w  " #vec2 ".s, " #preg "/z, [" #areg "," #avec2 ".s, UXTW #2]\n\t"

#define LDR_NOADDR_S(vec1, preg) " ld1rw  " #vec1 ".s, " #preg "/z"
#define OA_S(areg, offset) ",[" #areg ", #" #offset "]"
...
#define LOAD2VEC_DIST_S(vec1, vec2, preg, areg) \
  LDR_NOADDR_S(vec1, preg)                      \
  OA_S(areg, 0) "\n\t" LDR_NOADDR_S(vec2, preg) OA_S(areg, 4) "\n\t"

#define LOAD4VEC_DIST_S(vec1, vec2, vec3, vec4, preg, areg) \
  LDR_NOADDR_S(vec1, preg)                                  \
  OA_S(areg, 0) "\n\t" LDR_NOADDR_S(vec2, preg)             \
      OA_S(areg, 4) "\n\t" LDR_NOADDR_S(vec3, preg)         \
          OA_S(areg, 8) "\n\t" LDR_NOADDR_S(vec4, preg) OA_S(areg, 12) "\n\t"

#define LOAD8VEC_DIST_S(vec1, vec2, vec3, vec4, vec5, vec6, vec7, vec8, preg, \
                        areg)                                                 \
  LDR_NOADDR_S(vec1, preg)                                                    \
  OA_S(areg, 0) "\n\t" LDR_NOADDR_S(vec2, preg)                               \
      OA_S(areg, 4) "\n\t" LDR_NOADDR_S(vec3, preg)                           \
          OA_S(areg, 8) "\n\t" LDR_NOADDR_S(vec4, preg)                       \
              OA_S(areg, 12) "\n\t" LDR_NOADDR_S(vec5, preg)                  \
                  OA_S(areg, 16) "\n\t" LDR_NOADDR_S(vec6, preg)              \
                      OA_S(areg, 20) "\n\t" LDR_NOADDR_S(vec7, preg)          \
                          OA_S(areg, 24) "\n\t" LDR_NOADDR_S(vec8, preg)      \
                              OA_S(areg, 28) "\n\t"
...
#define LOAD2VEC_VOFF_C(vec1, vec2, preg, areg, off1, off2) \
  " ld1w  " #vec1 ".s, " #preg "/z, [" #areg ",#" #off1     \
  ",   MUL VL]           \n\t"                              \
  " ld1w  " #vec2 ".s, " #preg "/z, [" #areg ",#" #off2 ", MUL VL]\n\t"

#define LDC2_DIST_NOADDR_S(vec1, vec2, preg) " ld1rd {" #vec1 ".d}, " #preg "/z"
...
#define LOAD2VEC_DIST_C(vec1, vec2, preg, areg) \
  LOAD2VEC_DIST_OFF_C(vec1, vec2, preg, areg, 0)

#define LOAD4VEC_DIST_OFF_C(vec1, vec2, vec3, vec4, preg, areg, off1, off2, \
                            off3, off4)                                     \
...
#define LOAD8VEC_DIST_C(vec1, vec2, vec3, vec4, vec5, vec6, vec7, vec8, preg, \
                        areg)                                                 \
  LOAD4VEC_DIST_OFF_C(vec1, vec2, vec3, vec4, preg, areg, 0, 4, 8, 12)        \
...
#define MKINDC_2VEC_S(ivec1, ivec2, stridereg, nelemreg, sparereg) \
  ...
  " add " #ivec2 ".s," #ivec2                                      \
  ".s,#1\n\t"                                                      \
  " zip1 " #ivec1 ".s," #ivec1 ".s," #ivec2                        \
  ".s\n\t"                                                         \
  " mul " #sparereg ", " #sparereg ", " #nelemreg                  \
  "\n\t"                                                           \
  " lsr " #sparereg ", " #sparereg                                 \
  ", #1\n\t"                                                       \
...
#define LOAD2VEC_VOFF_C(vec1, vec2, preg, areg, off1, off2)          \
  " ld2w {" #vec1 ".s," #vec2 ".s}, " #preg "/z, [" #areg ",#" #off1 \
  ", MUL VL]\n\t"

#define LOAD2VEC_DIST_OFF_C(vec_r, vec_i, preg, areg, off1, off2) \
  LDR_NOADDR_S(vec_r, preg)                                       \
  OA_S(areg, off1) "\n\t" LDR_NOADDR_S(vec_i, preg) OA_S(areg, off2) "\n\t"

#define LOAD2VEC_DIST_C(vec_r, vec_i, preg, areg) \
  LOAD2VEC_DIST_S(vec_r, vec_i, preg, areg)
#define LOAD4VEC_DIST_C(vec1_r, vec1_i, vec2_r, vec2_i, preg, areg) \
  LOAD4VEC_DIST_S(vec1_r, vec1_i, vec2_r, vec2_i, preg, areg)
#define LOAD8VEC_DIST_C(vec1_r, vec1_i, vec2_r, vec2_i, vec3_r, vec3_i,   \
                        vec4_r, vec4_i, preg, areg)                       \
  LOAD8VEC_DIST_S(vec1_r, vec1_i, vec2_r, vec2_i, vec3_r, vec3_i, vec4_r, \
                  vec4_i, preg, areg)
...
#define LOAD2VEC_CONT_C(vec1, vec2, preg, areg, avec1, avec2) \
  LOAD2VEC_C(vec1, vec2, preg, areg)
...
#define STOR2VEC_CONT_S(vec1, vec2, preg, areg, avec1, avec2) \
  STOR2VEC_S(vec1, vec2, preg, areg)

// Store 2 vectors with generic indexing (scatter-store)
#define STOR2VEC_GENI_S(vec1, vec2, preg, areg, avec1, avec2) \
  " st1w  {" #vec1 ".s}, " #preg ", [" #areg "," #avec1       \
  ".s, UXTW #2]\n\t"                                          \
  " st1w  {" #vec2 ".s}, " #preg ", [" #areg "," #avec2 ".s, UXTW #2]\n\t"

#define STOR2VEC_CONT_C(vec1, vec2, preg, areg, avec1, avec2) \
  STORC2VEC_S(vec1, vec2, preg, areg)
...
#define PREF64(type, areg, off) " prfm " #type ",[" #areg ",#" #off "]\n\t"
#else
#define PREF64(type, areg, off)
#endif

#if defined(PREFETCH256)
#define PREF256(type, areg, off) " prfm " #type ",[" #areg ",#" #off "]\n\t"
#else
#define PREF256(type, areg, off)
#endif

#if defined(PREFETCH64) || defined(PREFETCH256)
#define PREFANY(type, areg, off) " prfm " #type ",[" #areg ",#" #off "]\n\t"
#else
...
#define ZEROVEC(vec1) " dup " #vec1 ".s, #0\n\t"
...
#define MLA1ROW_LA_LB_S(cvec, avec, bvec, preg, nextavec, aareg, avoff, bareg, \
                        bboff)                                                 \
  MLA1ROW_S(cvec, avec, bvec, preg)                                            \
  " ld1w   " #nextavec ".s, " #preg "/z, [" #aareg ", #" #avoff                \
  ", MUL VL]\n\t"                                                              \
  " ld1rw  " #bvec ".s, " #preg "/z, [" #bareg ",#" #bboff "]\n\t"

#define MLA2ROW_LA_LB_S(c1, c2, a1, a2, bvec, preg, nextavec, aareg, avoff, \
                        bareg, bboff)                                       \
  MLA2ROW_S(c1, c2, a1, a2, bvec, preg)                                     \
  " ld1w   " #nextavec ".s, " #preg "/z, [" #aareg ", #" #avoff             \
  ", MUL VL]\n\t"                                                           \
...
#define MLA2X2ROW_LA_LB_S(c11, c12, c21, c22, a1, a2, bvec1, bvec2, preg, \
                          nextavec, aareg, avoff, bareg, bboff1, bboff2)  \
  MLA2ROW_S(c11, c12, a1, a2, bvec1, preg)                                \
  MLA2ROW_S(c21, c22, a1, a2, bvec2, preg)                                \
  " ld1w   " #nextavec ".s, " #preg "/z, [" #aareg ", #" #avoff           \
  ", MUL VL]\n\t"                                                         \
...
#define MLA4ROW_LA_LB_S(c1, c2, c3, c4, a1, a2, a3, a4, bvec, preg, nextavec, \
                        aareg, avoff, bareg, bboff)                           \
  MLA4ROW_S(c1, c2, c3, c4, a1, a2, a3, a4, bvec, preg)                       \
  " ld1w   " #nextavec ".s, " #preg "/z, [" #aareg ", #" #avoff               \
  ", MUL VL]\n\t"                                                             \
  " ld1rw  " #bvec ".s, " #preg "/z, [" #bareg ",#" #bboff "]\n\t"

#define MLA2X2ROW_LB_S(c11, c12, c21, c22, a1, a2, bvec1, bvec2, preg, bareg, \
                       bboff1, bboff2)                                        \
  MLA2ROW_S(c11, c12, a1, a2, bvec1, preg)                                    \
  MLA2ROW_S(c21, c22, a1, a2, bvec2, preg)                                    \
  " ld1rw  " #bvec1 ".s, " #preg "/z, [" #bareg ",#" #bboff1                  \
...
#if defined(USE_SVE_CMLA_INSTRUCTION)
#define MLA1ROW_ILV_C(cvec1, cvec2, avec1, avec2, bvec1, bvec2, preg, ilv1,    \
                      ilv2, ilv3, ilv4)                                        \
  ...
  "\n\t"
#else
#define MLA1ROW_ILV_C(cvec_r, cvec_i, avec_r, avec_i, bvec_r, bvec_i, preg, \
                      ilv1, ilv2, ilv3, ilv4)                               \
...
#define MLA1ROW_ILV_LA_LB_C(c1, c2, a1, a2, bvec1, bvec2, preg, ilv1, ilv2,  \
                            ilv3, ilv4, nextavec1, nextavec2, aareg, avoff1, \
                            avoff2, bareg, bboff1, bboff2)                   \
...
#define MLA1ROW_ILV_LB_C(c1, c2, a1, a2, bvec1, bvec2, preg, ilv1, ilv2, ilv3, \
                         ilv4, bareg, bboff1, bboff2)                          \
  MLA1ROW_ILV_C(c1, c2, a1, a2, bvec1, bvec2, preg, ilv1, ilv2, ilv3, ilv4)    \
  LOAD2VEC_DIST_OFF_C(bvec1, bvec2, preg, bareg, bboff1, bboff2)

#define MLA1ROW_LA_LB_C(c1, c2, a1, a2, bvec1, bvec2, preg, nextavec1,    \
                        nextavec2, aareg, avoff1, avoff2, bareg, bboff1,  \
                        bboff2)                                           \
  MLA1ROW_ILV_LA_LB_C(c1, c2, a1, a2, bvec1, bvec2, preg, "", "", "", "", \
                      nextavec1, nextavec2, aareg, avoff1, avoff2, bareg, \
                      bboff1, bboff2)

#define MLA1ROW_LB_C(c1, c2, a1, a2, bvec1, bvec2, preg, bareg, bboff1,       \
                     bboff2)                                                  \
  MLA1ROW_ILV_LB_C(c1, c2, a1, a2, bvec1, bvec2, preg, "", "", "", "", bareg, \
                   bboff1, bboff2)

#define MLA2ROW_C(cvec1, cvec2, cvec3, cvec4, avec1, avec2, avec3, avec4, \
                  bvec1, bvec2, preg)                                     \
  MLA1ROW_C(cvec1, cvec2, avec1, avec2, bvec1, bvec2, preg)               \
  MLA1ROW_C(cvec3, cvec4, avec3, avec4, bvec1, bvec2, preg)

#define PFL1(areg, preg, offset) \
  " prfd pldl1keep, " #preg ", [" #areg ", #" #offset ", MUL VL]\n\t"

// Check if a complex number is 0
// TODO: Only first complex number needs to be checked,
#if defined(USE_SVE_CMLA_INSTRUCTION)
#define CMPCZB_S(vec1, vec2, label) \
  " fcmeq p1.s, p0/z, " #vec1       \
  ".s, #0.0\n\t"                    \
  " nots p1.b, p0/z, p1.b\n\t"      \
  " b.none " label "\n\t"
#else
...
#define FINC_2COL(fsuf, addressing, c0, c1, c2, c3, ca0, ca1, avec0, avec1, \
                  alpha, beta, acc0, acc1, acc2, acc3, labelnr)             \
...
// Contiguous memory (CONT) or generic index (GENI) specified by addressing
// labelnr for the beta case jumps
#define FINC_4COL_1X(fsuf, addressing, c0, c1, c2, c3, ca0, ca1, ca2, ca3, \
                     avec, alpha, beta, acc0, acc1, acc2, acc3, labelnr)   \
  ZERO4VEC_S(c0, c1, c2, c3)                                               \
  "                                            \n\t"\
" fcmp d" #beta ",#0.0                       \n\t"\
" beq .S" #fsuf "BETAZERO" #addressing "COLSTOREDS" #labelnr "       \n\t"\
...
// Contiguous memory (CONT) or generic index (GENI) specified by addressing
// labelnr for the beta case jumps
#define FINC_4COL(fsuf, addressing, c0, c1, c2, c3, c4, c5, c6, c7, ca0, ca1,  \
                  ca2, ca3, avec0, avec1, alpha, beta, acc0, acc1, acc2, acc3, \
                  acc4, acc5, acc6, acc7, labelnr)                             \
...
// Contiguous memory (CONT) or generic index (GENI) specified by addressing
// labelnr for the beta case jumps
// TODO: csX are needed because there is no fcmul instruction - find a better
// way without using
//       8 additional vector registers
#define CFINC_4COL(fsuf, addressing, c0, c1, c2, c3, c4, c5, c6, c7, cs0, cs1, \
                   cs2, cs3, cs4, cs5, cs6, cs7, ca0, ca1, ca2, ca3, avec1,    \
                   avec2, alpha1, alpha2, beta1, beta2, acc0, acc1, acc2,      \
                   acc3, acc4, acc5, acc6, acc7, labelnr)                      \
  ZERO8VEC_S(c0, c1, c2, c3, c4, c5, c6, c7)                                   \
  "                                            \n\t"\
