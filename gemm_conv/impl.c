...
inline void copy(int valid_len, int stride, float *out_ptr, float *in_ptr) {
  ...
  if (0 != stride) {
    svint32_t idx = svindex_s32(0, stride);
    do {
      svst1_f32(pg0, out_ptr, svld1_gather_s32offset_f32(pg0, in_ptr, idx));
      svst1_f32(pg1, out_ptr + vec_len,
                svld1_gather_s32offset_f32(pg1, in_ptr + vec_len, idx));
      svst1_f32(pg2, out_ptr + vec_len2,
                svld1_gather_s32offset_f32(pg2, in_ptr + vec_len2, idx));
      svst1_f32(pg3, out_ptr + vec_len3,
                svld1_gather_s32offset_f32(pg3, in_ptr + vec_len3, idx));
      x += vec_len4;
...
void im2col(float *src, float *dst, const int stride, const int pad,
...
#pragma omp parallel for num_threads(max_threads) private(kernel_row, \
                                                          kernel_col)
    for (kernel_total = 0; kernel_total < kernel_width * kernel_height;
         ++kernel_total) {
      ...
      kernel_col = kernel_total % kernel_width;
      {
        ...
        for (int out_row = 0; out_row < out_height;
             ++out_row, input_row += stride) {
          if (input_row < 0 || input_row >= height) {
            continue;
          }
          float *dst_offset =
              dst_ptr + (kernel_row * kernel_width + kernel_col) *
                            (out_width * out_height);
          // * here could be optimized
#ifndef __ARM_FEATURE_SVE
          int input_col = -pad + kernel_col;
          for (int out_col = 0; out_col < out_width;
               ++out_col, input_col += stride) {
            if (input_col < 0 || input_col >= width) {
              continue;
            }
            dst_offset[out_row * out_width + out_col] =
                src_ptr[input_row * width + input_col];
          }
#else
          ...
          int right_offset = (kernel_width - (kernel_col + 1) > pad
                                  ? kernel_width - (kernel_col + 1) - pad
                                  : 0);
          ...
          // kernel_col, left_offset, right_offset); copy(valid_len, out_ptr,
          // in_ptr);
          const svbool_t all_true_pg = svptrue_b32();
...
void conv_im2col_gemm_f32(float *src, float *weights, float *dst, int batches,
                          int channel, int height, int width, int out_channel,
  ...
  float *trans_image =
      (float *)malloc(out_height * out_width * channel * kernel_height *
                      kernel_width * sizeof(float));
  im2col(src, trans_image, stride, pad, channel, height, width, kernel_height,
